...
static bool bt_app_commmon_is_meet_security_level(bt_gap_le_bonding_info_t *link_bond_info, bt_gap_le_smp_pairing_config_t *current_pairing_config)
{
    bool result = true;
    if ((current_pairing_config->maximum_encryption_key_size > link_bond_info->key_size) ||
        ((current_pairing_config->auth_req & BT_GAP_LE_SMP_AUTH_REQ_MITM) && (!(link_bond_info->key_security_mode & BT_GAP_LE_SECURITY_AUTHENTICATION_MASK))) ||
        ((current_pairing_config->auth_req & BT_GAP_LE_SMP_AUTH_REQ_SECURE_CONNECTION) && (!(link_bond_info->key_security_mode & BT_GAP_LE_SECURITY_LESC_MASK)))) {
        result = false;
    }
    LOG_MSGID_I(BT_APP, "[APP][COMMON] meet security level result", 1, result);
    return result;
}
...
static bt_status_t bt_app_common_get_pairing_config(bt_gap_le_bonding_start_ind_t *ind)
{
    LOG_MSGID_I(BT_APP, "[APP][COMMON] connection handle = %02x, is_previously_bonded(%d)", 2, ind->handle, ind->is_previously_bonded);
        ...
        if ((ind->is_previously_bonded) && ((conn_info->role == BT_ROLE_SLAVE) ||
                        ((bond_info != NULL) && (!bt_app_commmon_is_meet_security_level(&bond_info->info, &pairing_config_le_audio))))) {
...
#if !defined(AIR_BT_SOURCE_ENABLE) && !defined(AIR_PURE_GAMING_ENABLE)
