    ...
    size_t data_len)
{
    ...
    switch (event_id) {
        ...
        case APPS_EVENTS_USB_AUDIO_SAMPLE_RATE: {
            ...
            uint32_t idx = get_idx_by_port(sr_info->port.port_type, sr_info->port.port_num);
            ...
            if (s_samples[idx] != new_rate) {
                ...
                void* handle = get_handle_by_port(sr_info->port.port_type, sr_info->port.port_num);
        ...
        case APPS_EVENTS_USB_AUDIO_SAMPLE_SIZE: {
            ...
            uint32_t idx = get_idx_by_port(size_info->port.port_type, size_info->port.port_num);
            if (size_info->size > 0) {
        ...
        case APPS_EVENTS_USB_AUDIO_CHANNEL: {
            app_events_usb_channel_t *channel_info = (app_events_usb_channel_t*)&extra_data;
            uint32_t idx = get_idx_by_port(channel_info->port.port_type, channel_info->port.port_num);

            if (channel_info->channel > 0) {
                s_channels[idx] = channel_info->channel;
                /* set sample size -> set channels -> start. */
                void* handle = get_handle_by_port(channel_info->port.port_type, channel_info->port.port_num);
                if (!s_started[idx] || !s_usb_audio_mode) {
                    app_audio_trans_mgr_deinit_audio(handle);
                    app_audio_trans_mgr_init_audio(handle);
                } else {
