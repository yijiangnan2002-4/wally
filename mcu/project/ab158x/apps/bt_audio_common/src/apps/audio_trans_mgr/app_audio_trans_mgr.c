...
#endif
#if defined(AIR_DCHS_MODE_MASTER_ENABLE) || (defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE) && defined(AIR_BTA_IC_STEREO_HIGH_G3))
#define AIR_2CHIPS_WIRED_AUDIO_SUSPEND_SALVE_BT
#endif
#if defined(AIR_DCHS_MODE_SLAVE_ENABLE) || (defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE) && defined(AIR_BTA_IC_STEREO_HIGH_G3))
#define AIR_2CHIPS_BT_SUSPEND_BY_WIRED_AUDIO
#endif


#if defined(AIR_2CHIPS_WIRED_AUDIO_SUSPEND_SALVE_BT) || defined(AIR_2CHIPS_BT_SUSPEND_BY_WIRED_AUDIO)
...
#else
                audio_source_control_cmd(u_ctx->audio_source_ctrl_handle, AUDIO_SOURCE_CONTROL_CMD_RELEASE_RES, AUDIO_SOURCE_CONTROL_CMD_DEST_LOCAL);
#endif
                if (now_sta) {
        ...
        case AUDIO_SRC_SRV_EVENT_SUSPEND:
            ...
            if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN1 || u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN2) {
                ...
                if (u_ctx->state == APP_AUDIO_TRANS_MGR_TRANS_STA_STARTING) {
                    ...
                    __set_trans_state(u_ctx, APP_AUDIO_TRANS_MGR_TRANS_STA_SUSPEND);
                    __send_self_cmd(u_ctx, APP_AUDIO_TRANS_MGR_CMD_STOP);
                }
    ...
    switch (ev_type) {
        case AUDIO_SOURCE_CONTROL_EVENT_RES_CTRL: {
            ...
            switch (res_ev) {
                case AUDIO_SRC_SRV_EVENT_TAKE_ALREADY:
                case AUDIO_SRC_SRV_EVENT_TAKE_SUCCESS:
                    #if defined(AIR_BTA_IC_STEREO_HIGH_G3) && defined(AIR_2CHIPS_WIRED_AUDIO_SUSPEND_SALVE_BT)
                    if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN1 ||
                        u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN2 ||
                        u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_LINE_IN) {
                        uint16_t extra_data = (uint8_t)u_ctx->usr_info.type;
...
void __app_audio_trans_mgr_init_audio(app_audio_trans_mgr_usr_ctx_t *u_ctx)
{
    ...
    if (u_ctx->audio_source_ctrl_handle == NULL && u_ctx->usr_info.ctrl_type == APP_AUDIO_TRANS_MGR_CTRL_TYPE_AUD_SRC_CTRL) {
        /* for special case, usb in & usb in2 use same handle */
        if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN1 && s_ctx.usrs[APP_AUDIO_TRANS_MGR_USR_USB_IN2].audio_source_ctrl_handle != NULL) {
            u_ctx->audio_source_ctrl_handle = s_ctx.usrs[APP_AUDIO_TRANS_MGR_USR_USB_IN2].audio_source_ctrl_handle;
        } else if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN2 && s_ctx.usrs[APP_AUDIO_TRANS_MGR_USR_USB_IN1].audio_source_ctrl_handle != NULL) {
    ...
    if (u_ctx->pseudo_device_handle == NULL && u_ctx->usr_info.ctrl_type == APP_AUDIO_TRANS_MGR_CTRL_TYPE_SEUDO_DEVICE) {
        if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN1 && s_ctx.usrs[APP_AUDIO_TRANS_MGR_USR_USB_IN2].pseudo_device_handle != NULL) {
            ...
            u_ctx->pseudo_device_handle = s_ctx.usrs[APP_AUDIO_TRANS_MGR_USR_USB_IN1].pseudo_device_handle;
        } else {
    ...
    if (u_ctx->usr_info.ctrl_type == APP_AUDIO_TRANS_MGR_CTRL_TYPE_SEUDO_DEVICE) {
        ...
        if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN1 || u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_USB_IN2) {
            if (u_ctx->pseudo_device_handle->state == AUDIO_SRC_SRV_STATE_PREPARE_PLAY) {
                need_take_resource = false;
            }
        }
        __set_trans_state(u_ctx, APP_AUDIO_TRANS_MGR_TRANS_STA_STARTING);
        if (need_take_resource) {
...
#if defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE) || defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_SLAVE_ENABLE)
    if (u_ctx->usr_info.ctrl_type == APP_AUDIO_TRANS_MGR_CTRL_TYPE_AUD_SRC_CTRL) {
        audio_source_control_cmd(u_ctx->audio_source_ctrl_handle, AUDIO_SOURCE_CONTROL_CMD_DEL_WAITTING_LIST, AUDIO_SOURCE_CONTROL_CMD_DEST_LOCAL);
    }
#endif
...
{
    bool ret = false;
#if defined(AIR_2CHIPS_WIRED_AUDIO_SUSPEND_SALVE_BT)
...
#if (defined(AIR_DUAL_CHIP_MIXING_MODE_ROLE_MASTER_ENABLE) && defined(AIR_BTA_IC_STEREO_HIGH_G3))
    /* workaround, volume change step by step now only. */
    if (u_ctx->usr_info.type == APP_AUDIO_TRANS_MGR_USR_LINE_IN) {
        if (l == u_ctx->usr_info.volume_l) {
            return;
        }
        audio_source_control_cmd(u_ctx->audio_source_ctrl_handle,
            l > u_ctx->usr_info.volume_l ? AUDIO_SOURCE_CONTROL_CMD_VOLUME_UP : AUDIO_SOURCE_CONTROL_CMD_VOLUME_DOWN,
