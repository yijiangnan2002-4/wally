...
battery_event_shutdown_state_t apps_events_battery_get_shutdown_state()
{
    ...
    s_shutdown_state = calculate_shutdown_state(battery_voltage);
    return s_shutdown_state;
}
...
void app_events_battery_set_shutdown_state(battery_event_shutdown_state_t current_state)
{
    if (s_shutdown_state != current_state) {
        s_shutdown_state = current_state;
        ui_shell_send_event(false, EVENT_PRIORITY_MIDDLE, EVENT_GROUP_UI_SHELL_BATTERY,
...
{
    if (APPS_EVENTS_BATTERY_SHUTDOWN_STATE_VOLTAGE_LOW == (battery_event_shutdown_state_t)shutdown_state) {
        ui_shell_send_event(false, EVENT_PRIORITY_MIDDLE, EVENT_GROUP_UI_SHELL_BATTERY,
                            APPS_EVENTS_BATTERY_SHUTDOWN_STATE_CHANGE, (void *)shutdown_state, 0, NULL, 0);
    }
    APPS_LOG_MSGID_I("[DCHS_battery] recv slave_shut_state:%d,", 1, shutdown_state);
}
...
static void _timer_callback_function(TimerHandle_t xTimer)
{
    ...
    if (shutdown_state != s_shutdown_state) {
        ...
        app_events_battery_set_shutdown_state(shutdown_state);
    }
...
static atci_status_t _battery_atci_update_voltage(atci_parse_cmd_param_t *parse_cmd)
{
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_EXECUTION:
            ...
            app_events_battery_set_shutdown_state(mock_shutdown_state);
//            ui_shell_send_event(false, EVENT_PRIORITY_MIDDLE, EVENT_GROUP_UI_SHELL_BATTERY,
