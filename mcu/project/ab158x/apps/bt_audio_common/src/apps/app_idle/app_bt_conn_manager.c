...
#ifdef APP_CONN_MGR_RECONNECT_SAVE_LAST_TWO_DEVICE
    uint8_t old_total_num = old_edr_num + old_lea_num + old_ull2_num;
    uint8_t new_total_num = app_bt_conn_ctx.edr_num + app_bt_conn_ctx.lea_num + app_bt_conn_ctx.ull2_num;
    if (event == APP_BT_CONN_EVENT_EDR_CONNECTED || event == APP_BT_CONN_EVENT_LEA_CONNECTED || event == APP_BT_CONN_EVENT_ULL2_CONNECTED
        || (old_total_num == APP_BT_CONN_MAX_CONN_NUM + 1 && new_total_num == APP_BT_CONN_MAX_CONN_NUM)
        || (!support_emp && old_total_num == APP_BT_CONN_MAX_CONN_NUM && new_total_num == 1)) {
...
#else
    app_bt_conn_reconnect_info_t *info = &app_bt_conn_reconnect_info;
#endif
    bt_aws_mce_role_t role = bt_connection_manager_device_local_info_get_aws_role();
    uint8_t *addr0 = info->item[0].addr;
    uint8_t *addr1 = info->item[1].addr;
    APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] print_save_info, [%02X] connected_num=%d edr_num=%d lea_num=%d",
                     4, role, info->edr_num + info->le_num, info->edr_num, info->le_num);
    APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] print_save_info, link0=%d-%d-%08X%04X edr_profile_flag=0x%02X conn_state=0x%02X 0x%02X 0x%02X",
                     8, info->item[0].link_type, info->item[0].addr_type,
                     *((uint32_t *)(addr0 + 2)), *((uint16_t *)addr0),
                     info->item[0].edr_profile_flag, info->item[0].conn_state,
                     info->item[0].reserve1, info->item[0].reserve2);
    APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] print_save_info, link1=%d-%d-%08X%04X edr_profile_flag=0x%02X conn_state=0x%02X 0x%02X 0x%02X",
                     8, info->item[1].link_type, info->item[1].addr_type,
                     *((uint32_t *)(addr1 + 2)), *((uint16_t *)addr1),
                     info->item[1].edr_profile_flag, info->item[1].conn_state,
...
#ifdef APP_CONN_MGR_RECONNECT_SAVE_LAST_TWO_DEVICE
void app_bt_conn_manager_update(void)
{
    bt_aws_mce_role_t role = bt_connection_manager_device_local_info_get_aws_role();
    ...
    APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] update, save_num=%d connected_num=%d", 2, save_num, connected_num);
    if (save_num < connected_num || (save_num == connected_num && connected_num == APP_BT_CONN_MAX_CONN_NUM)) {
        ...
        for (int i = 0; i < connected_num; i++) {
            ...
            app_bt_conn_reconnect_save_info.item[i].link_type = link_info->connected_device[i].link_type;
            app_bt_conn_reconnect_save_info.item[i].addr_type = link_info->connected_device[i].remote_bd_type;
            memcpy(app_bt_conn_reconnect_save_info.item[i].addr, link_info->connected_device[i].remote_addr, BT_BD_ADDR_LEN);
        }
        ...
        const bt_device_manager_link_record_item_t *last_item = &link_info->connected_device[0];
...
void app_bt_conn_manager_save(void)
{
    ...
    app_bt_conn_reconnect_save_info.item[0].conn_state = APP_BT_CONN_STATE_NONE;
    app_bt_conn_reconnect_save_info.item[1].conn_state = APP_BT_CONN_STATE_NONE;
    const bt_device_manager_link_record_t *link_info = bt_device_manager_link_record_get_connected_link();
    for (int i = 0; i < APP_BT_CONN_MAX_CONN_NUM; i++) {
        ...
        for (int j = 0; j < link_info->connected_num; j++) {
            ...
            if (link_type == conn_link_type && memcmp(addr, conn_addr, BT_BD_ADDR_LEN) == 0) {
                app_bt_conn_reconnect_save_info.item[i].conn_state = APP_BT_CONN_STATE_CONNECTED;
                break;
            }
        }
    }
...
void app_bt_conn_manager_restore(void)
{
    ...
    if (app_bt_conn_reconnect_info.edr_num == 1 && app_bt_conn_reconnect_info.le_num == 1
        && memcmp(app_bt_conn_reconnect_info.item[0].addr, app_bt_conn_reconnect_info.item[1].addr, BT_BD_ADDR_LEN) == 0) {
        if (app_bt_conn_reconnect_info.item[0].conn_state == APP_BT_CONN_STATE_CONNECTED
            && app_bt_conn_reconnect_info.item[1].conn_state == APP_BT_CONN_STATE_NONE) {
            dual_connection_keep_last_link_type = TRUE;
            APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] restore, dual_connection_keep_last_link_type", 0);
        } else if (app_bt_conn_reconnect_info.item[0].conn_state == APP_BT_CONN_STATE_NONE
                && app_bt_conn_reconnect_info.item[1].conn_state == APP_BT_CONN_STATE_CONNECTED) {
            APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] restore, dual_connection_exchange", 0);
            app_bt_conn_reconnect_item_t temp = {0};
            memcpy(&temp, &app_bt_conn_reconnect_info.item[0], sizeof(app_bt_conn_reconnect_item_t));
            memcpy(&app_bt_conn_reconnect_info.item[0], &app_bt_conn_reconnect_info.item[1], sizeof(app_bt_conn_reconnect_item_t));
            memcpy(&app_bt_conn_reconnect_info.item[1], &temp, sizeof(app_bt_conn_reconnect_item_t));
            dual_connection_keep_last_link_type = TRUE;
        } else if (app_bt_conn_reconnect_info.item[0].conn_state == APP_BT_CONN_STATE_CONNECTED
                && app_bt_conn_reconnect_info.item[1].conn_state == APP_BT_CONN_STATE_CONNECTED) {
            APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] restore, dual_connection_both_connected", 0);
        } else {
            APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] restore, dual_connection_both_disconnected", 0);
            if (app_bt_conn_reconnect_info.item[0].link_type == BT_DEVICE_MANAGER_LINK_TYPE_LE) {
    ...
    if ((!support_emp || dual_connection_keep_last_link_type)
        && (app_bt_conn_reconnect_info.edr_num + app_bt_conn_reconnect_info.le_num == APP_BT_CONN_MAX_CONN_NUM)) {
        memset(&app_bt_conn_reconnect_info.item[1], 0, sizeof(app_bt_conn_reconnect_item_t));
        if (app_bt_conn_reconnect_info.item[0].link_type == BT_DEVICE_MANAGER_LINK_TYPE_EDR) {
            app_bt_conn_reconnect_info.edr_num = 1;
            app_bt_conn_reconnect_info.le_num = 0;
        } else {
            app_bt_conn_reconnect_info.edr_num = 0;
            app_bt_conn_reconnect_info.le_num = 1;
        }
    }

    // Clear conn_state
    app_bt_conn_reconnect_info.item[0].conn_state = APP_BT_CONN_STATE_NONE;
    app_bt_conn_reconnect_info.item[1].conn_state = APP_BT_CONN_STATE_NONE;
    app_bt_conn_reconnect_save_info.item[0].conn_state = APP_BT_CONN_STATE_NONE;
    app_bt_conn_reconnect_save_info.item[1].conn_state = APP_BT_CONN_STATE_NONE;
#endif
    ...
    if (le_num == 0 && edr_num == 0) {
...
#ifndef APP_CONN_MGR_RECONNECT_SAVE_LAST_TWO_DEVICE
        ...
        if (edr_num == 1) {
            ...
            for (int i = 0; i < APP_BT_CONN_MAX_CONN_NUM; i++) {
                if (app_bt_conn_reconnect_info.item[i].link_type == BT_DEVICE_MANAGER_LINK_TYPE_EDR) {
                    bool ret = app_bt_conn_mgr_connect_edr(app_bt_conn_reconnect_info.item[i].addr,
        ...
        uint8_t reconnect_edr_num = 0;
        app_bt_conn_reconnect_flag = TRUE;
        ui_shell_send_event(FALSE, EVENT_PRIORITY_HIGH, EVENT_GROUP_UI_SHELL_APP_CONN_MGR,
                            APP_CONN_MGR_EVENT_RECONNECT_TIME_OUT, NULL, 0, NULL, APP_CONN_RECONNECT_TIMER);

        // EDR - Reconnect one/two
        // when Reconnect one (edr_num==1), not exist two BT_DEVICE_MANAGER_LINK_TYPE_EDR
        for (int i = 0; i < APP_BT_CONN_MAX_CONN_NUM; i++) {
            if (app_bt_conn_reconnect_info.item[i].link_type == BT_DEVICE_MANAGER_LINK_TYPE_EDR) {
                bool ret = app_bt_conn_mgr_connect_edr(app_bt_conn_reconnect_info.item[i].addr,
                                                       app_bt_conn_reconnect_info.item[i].edr_profile_flag);
        ...
        if (reconnect_edr_num == 0) {
            app_bt_conn_mgr_reconnect_complete(FALSE);
        } else if (edr_num == 2 && reconnect_edr_num == 1) {
            app_bt_conn_reconnect_info.edr_num = 1;
        }
    } else {
        configASSERT(0);
...
{
    APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] active_reconnect_edr", 0);
#ifdef APP_CONN_MGR_RECONNECT_SAVE_LAST_TWO_DEVICE
    app_bt_conn_mgr_print_reconnect_save_info();
    ...
    if (app_bt_conn_reconnect_save_info.edr_num == 1
        ...
        APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] ignore 2nd same phone EDR addr", 0);
        return;
    }
    ...
    for (int i = 0; i < APP_BT_CONN_MAX_CONN_NUM; i++) {
        if (app_bt_conn_reconnect_save_info.item[i].link_type == BT_DEVICE_MANAGER_LINK_TYPE_EDR) {
            ...
            APPS_LOG_MSGID_W(LOG_TAG"[Reconnect] link_lost_timeout_reconnect_edr, reconnect saved EDR [%d]",
                             1, i);
        }
    }
#endif
            ...
            if (remote_update != NULL) {
                ...
                if (role == BT_AWS_MCE_ROLE_AGENT || role == BT_AWS_MCE_ROLE_NONE) {
                    ...
                    if (remote_update->pre_acl_state != BT_CM_ACL_LINK_ENCRYPTED
                        ...
                        APPS_LOG_MSGID_I(LOG_TAG" BT_CM event, Encrypted PNP status=0x%08X VID=%04X PID=%04X",
                                         3, pnp_status, pnp_info.vender_id, pnp_info.product_id);
    ...
    switch (event) {
        ...
        case BT_DEVICE_MANAGER_POWER_EVT_CLASSIC_ACTIVE_COMPLETE: {
            if (BT_DEVICE_MANAGER_POWER_STATUS_SUCCESS == status) {
                ...
                uint32_t pair_num = bt_device_manager_get_paired_number();
                APPS_LOG_MSGID_I(LOG_TAG"[Reconnect] BT DM event, CLASSIC POWER ON power_state=%d classic_off_num=%d pair_num=%d save_info_edr_num=%d edr=%d le=%d",
                                 6, power_state, app_bt_conn_classic_off_device_num, pair_num,
                                 app_bt_conn_reconnect_save_info.edr_num,
...
#if 1       // Disable - No need to restore when only BT classic reboot & no disconnect EDR
#ifdef APP_CONN_MGR_RECONNECT_SAVE_LAST_TWO_DEVICE
                else if (app_bt_conn_reconnect_save_info.edr_num > 0
                         && app_bt_conn_reconnect_info.edr_num == 0 && app_bt_conn_reconnect_info.le_num == 0) {
...
#if defined(AIR_LE_AUDIO_ENABLE) || defined(AIR_BLE_ULTRA_LOW_LATENCY_WITH_HID_ENABLE)
    extern bool app_lea_adv_add_ull_data;
    is_ull2 = (is_ull2 && app_lea_adv_add_ull_data);
#endif
#endif
#ifdef AIR_LE_AUDIO_ENABLE
    // Note: need to wait bond->discovery done, maybe fail
