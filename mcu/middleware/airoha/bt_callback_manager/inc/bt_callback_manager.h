...
typedef enum {
    ...
    bt_callback_type_gap_le_get_local_key_by_handle,             /**< Cannot have more than one callback registered for this callback type. The prototype of the callback function for this callback type is defined as #bt_gap_le_get_local_key_by_handle(). */
...
/**
 * @brief     This function adds a L2CAP LE user config to the L2CAP LE stack.
 * @param[in] config   is the pointer to the config of the customer profile.
 * @return             #BT_STATUS_SUCCESS, if the config was added successfully.
 *                     #BT_STATUS_FAIL, if the maximum number of records, as defined by #BT_CALLBACK_MANAGER_L2CAP_LE_USER_PROFILE_MAX in bt_callback_manager_config.h, has been reached.
 */
