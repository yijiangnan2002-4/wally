...
void atci_captouch_send_response(int32_t *len1, atci_response_t *response)
{
    ...
    while (1) {
        ...
        if (ret == HAL_CAPTOUCH_STATUS_NO_EVENT) {
            ret_len1 =  snprintf((char *)(response->response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, \
                                 "Buffer empty");
            atci_captouch_send_response(&ret_len1, response);
            break;
        }
    ...
    */
    ...
    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_TESTING:
            ...
            */
            strncpy((char *)response->response_buf, "ATCI_CMD_MODE_TESTING", strlen("ATCI_CMD_MODE_TESTING") + 1);
            response->response_len = strlen((char *)response->response_buf);
        ...
        case ATCI_CMD_MODE_EXECUTION:
            strncpy((char *)response->response_buf, "ATCI_CMD_MODE_EXECUTION", strlen("ATCI_CMD_MODE_EXECUTION") + 1);
            response->response_len = strlen((char *)response->response_buf);
            response->response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            atci_send_response(response);
                ...
                if ((*(char *)param2 == 'W') || (*(char *)param2 == 'w')) {
                    ...
                    for (i = 0; i < 0x28; i++) {
                        ...
                        if (((i >= 0x2) && (i <= 0x11)) || (i == 0x1d) || (i == 0x1e)) {
                            ...
                            ret_len1 = snprintf((char *)(response->response_buf + ret_len2), ATCI_UART_TX_FIFO_BUFFER_SIZE, \
            ...
            */
            if (strncmp(param1, "AT+ECPT=SET_LP", strlen("AT+ECPT=SET_LP")) == 0) {
                ...
                if (config_data[0] == 0) {
                    ...
                    ret_len1 =  snprintf((char *)(response->response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, "set to normal mode done!\r\n");
                } else if (config_data[0] == 1) {
                    hal_captouch_lowpower_control(HAL_CAPTOUCH_MODE_LOWPOWER);
                    ret_len1 =  snprintf((char *)(response->response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, "set to lowpoer mode done!\r\n");
                } else {
                    ret_len1 =  snprintf((char *)(response->response_buf), ATCI_UART_TX_FIFO_BUFFER_SIZE, "lowpoer mode data=%d wrong,should <2\r\n", (int)config_data[0]);
                }
            ...
            */
            if (strncmp(param1, "AT+ECPT=CAL_KEY", strlen("AT+ECPT=CAL_KEY")) == 0) {
                ...
                atci_captouch_send_response((int32_t *)&ret_len2, response);
