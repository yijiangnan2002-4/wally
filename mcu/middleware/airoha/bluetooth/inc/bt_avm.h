...
#define BT_AVM_RX_GAIN_SCENARIO_SCAN    (0x1 << 0)
#define BT_AVM_RX_GAIN_SCENARIO_INIT    (0x1 << 1)
typedef uint8_t bt_avm_rx_gain_secnario_mask_t;

#define BT_AVM_ULTRA_HIGH_RX_GAIN       (0x7F)
#define BT_AVM_HIGH_RX_GAIN             (0x6D)
#define BT_AVM_MIDDLE_HING_GAIN         (0x5B)
#define BT_AVM_MIDDLE_GAIN              (0x49)
#define BT_AVM_LOW_GAIN                 (0x37)
#define BT_AVM_ULTRA_LOW_GAIN           (0x25)
#define BT_AVM_NO_FIXED_RX_GAIN         (0x00)
typedef uint8_t bt_avm_fixed_rx_gain_t;
/**
...
typedef struct {
    uint8_t info[6];       /*TX power gc value*/
} bt_avm_vendor_set_tx_power_t;
...
/**
 * @brief     This function sets dongle mode to controller.
...
/**
 * @brief     This function sets antenna switch mode to controller.
 * @param[in] ans_enable is indicating that the antenna switch mode is enable or disable.
 * @param[in] gpio_cfg is gpio configure parameter using by antenna switch.
 * @return    #BT_STATUS_SUCCESS, set antenna switch mode successfully.
 *                #BT_STATUS_FAIL, set antenna switch mode failed.
 */
bt_status_t bt_avm_set_antenna_switch_mode(bool ans_enable, uint32_t gpio_cfg);

/**
 * @brief     This function sets front-end module mode to controller.
 * @param[in] fem_enable is indicating that front-end module mode is enable or disable.
 * @param[in] bypass_mode_enable is indicating that external front-end module enble or disbale bypass mode.
 * @param[in] gpio_cfg is gpio configure parameter using by external front-end module.
 * @return    #BT_STATUS_SUCCESS, set front-end module enble successfully.
 *                #BT_STATUS_FAIL,set front-end module enble failed.
 */
bt_status_t bt_avm_set_front_end_module_mode(bool fem_enable, bool bypass_mode_enable, uint32_t gpio_cfg);

/**
 * @brief     This function change local cod value.
 * @param[in] class_of_device is the value of cod
 * @return    #BT_STATUS_SUCCESS, cod successfully.
 *                #BT_STATUS_FAIL,cod front-end module enble failed.
 */
bt_status_t bt_avm_change_local_cod(uint32_t class_of_device);

/**
 * @brief     Tis function sets rx gain to controller.
 * @param[in] scenario_mask is the scenario to use fixed rx gain.
 * @param[in]  fixed_rx_gain is the rx gain value for the scenario.
 * @return    #BT_STATUS_SUCCESS, send vendor cmd success.
 *                #BT_STATUS_FAIL,set vendor cmd fail.
 */
bt_status_t bt_avm_set_rx_gain(bt_avm_rx_gain_secnario_mask_t scenario_mask, bt_avm_fixed_rx_gain_t fixed_rx_gain);

/**
 * @brief     Tis function sets rx TX power info to controller.
 * @param[in] power_info is the tx power gain value.
 * @return    #BT_STATUS_SUCCESS, send vendor cmd success.
 *                #BT_STATUS_FAIL,set vendor cmd fail.
 */
bt_status_t bt_avm_vendor_set_tx_power(bt_avm_vendor_set_tx_power_t *power_info);
BT_EXTERN_C_END
