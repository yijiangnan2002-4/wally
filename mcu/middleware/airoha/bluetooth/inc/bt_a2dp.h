...
typedef struct {
    ...
    uint8_t value[5];           /**< The codec value. It contains values specifically defined by the Vendor. */
...
typedef struct {
    ...
    uint16_t customer_feature_option;              /**< Feature options for customer. bit0: mHDT. */
...
* @param[in] codec   is the configured codec from remote, which is requested to check the validation by local end.
* @param[in] length  is the configured codec length from remote.
