...
#define BT_GAP_WRITE_LOCAL_NAME_CNF                 (BT_MODULE_GAP | 0x012D)    /**< This event indicates the confirmation of #bt_gap_write_local_name() with NULL payload. */
...
/**
 * @brief     This function is used to set the local device name.
 * @param[in] name        is a pointer that points to the device name buffer.
 * @param[in] name_length is the device name length, the max length is 248 octets.
 * @return               #BT_STATUS_SUCCESS, the operation completed successfully.
 *                       #BT_STATUS_FAIL, an error occurred, the operation failed.
 */
bt_status_t bt_gap_write_local_name(const char *name, uint32_t name_length);

/**
 * @brief     This function is used to get the ACL link handle via the remote device's bluetooth address.
 * @param[in] address     is the address of a connected device.
 * @return               the connected acl handle. #0 means it is invalid.
 */
bt_handle_t bt_gap_find_connection_handle(const bt_bd_addr_t *address);

/**
 * @brief     This function is used to enable or disable classic BT security connection only mode.
 * @param[in] enable     defines whether to enable or disable the security connection only mode.
 * @return               None.
 */
void        bt_gap_set_security_connection_only_mode(bool enable);
