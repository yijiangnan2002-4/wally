 ...
 */
#define BT_DEVICE_MANAGER_LE_CB_MAX_NUM (5)
...
/**
 * @brief         This function gets the bonding information via the LE link type.
 * @param[in]     link_type          is the LE link type.
 * @param[in]     infos              is the buffer to save the information of bonded LE devices, it must be provided by the application.
 * @param[in,out] count              is the input and output parameter. As an input parameter, it is the length of the information and the maximum number of
 *                                   bonded LE devices that the information can hold. As an output parameter, it is the actual number of bonded LE devices
 *                                   stored in the information upon the return of this function, which cannot exceed the length of the information.
 * @return                           BT_STATUS_SUCCESS, the information is successfully attained.
 *                                   BT_STATUS_FAIL, it failed to get the information
 */
...
/**
 * @brief         This function generates an RPA (Resolvable Private Address).
 * @param[in]     bond_info          is a pointer to a bonded LE device.
 * @param[in]     prand              is a pointer to a random number sequence which is used to generate the RPA.
 * @param[in]     prand_length       is the length of prand.
 * @param[out]    rpa                is a pointer to a piece of buffer which is used to get the RPA.
 * @return                           #BT_STATUS_SUCCESS if the RPA is generated successfully.
 *                                   #BT_STATUS_FAIL    if the RPA generation fails.
 */
bt_status_t bt_device_manager_le_generate_rpa(bt_device_manager_le_bonded_info_t *bond_info, uint8_t *prand, uint32_t prand_length, bt_bd_addr_t *rpa);
