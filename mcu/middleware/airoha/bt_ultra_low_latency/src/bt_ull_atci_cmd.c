...
#ifdef AIR_BLE_ULTRA_LOW_LATENCY_ENABLE
    if (0 == memcmp(string, ULL_STRPARAM("change_bitrate"))) {
        uint8_t codec_index;
...
*/
    ...
    if (0 == memcmp(string, ULL_STRPARAM("CLEAR_BONDING_LIST"))) {
...
#ifdef AIR_BLE_ULTRA_LOW_LATENCY_ENABLE
    if (0 == memcmp(string, ULL_STRPARAM("DL_AM_PLAY"))) {
...
uint8_t ab1585h_command_no=5;		// 0: BT status; 1:OTA staus; 2: version feed back
