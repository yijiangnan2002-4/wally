...
#else
#include "bl_common.h"
#define configASSERT(x) { if (!(x)) {bl_print(LOG_CRIT,"assert on mbedtls AES ALT\n\r"); while (1); }}
                           ...
                           unsigned int keybits)
{
    switch (keybits) {
        case 128:
            ctx->key_byte_length = 16;
            break;
        case 192:
            ctx->key_byte_length = 24;
            break;
        case 256:
            ctx->key_byte_length = 32;
            break;
        default :
                          ...
                          unsigned char output[16])
{
    ...
    if (mode == 0) { /* decrypt */
        ...
        } while (ret == -100 && count <= MBEDTLS_HW_ALT_RETRY_TIME);
    } else {
        do {
