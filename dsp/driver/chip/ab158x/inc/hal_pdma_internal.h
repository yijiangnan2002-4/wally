...
/**
 * @brief    get the the read pointer offset of virtual fifo
 * @param[in] channel,dma master name definition in enum vdma_channel_t
 * @param[out] *read_offset ,get the read point offset in virtual fifo RDPTR and PGMADDR register
 * @return if get the read point offset correctly, return VDMA_OK
 * @note
 * @warning
 * @par       VDMA_UART1TX
 * @code      vdma_get_hw_read_offset(VDMA_UART1TX, &read_point);
 * @endcode
 */
vdma_status_t vdma_get_hw_read_offset(vdma_channel_t channel, uint32_t *read_offset);
...
/**
 * @brief    get the the write pointer offset of virtual fifo
 * @param[in] channel,dma master name definition in enum vdma_channel_t
 * @param[out] *write_point ,get the write point offset in virtual fifo WRPTR and PGMADDR register
 * @return if get the read point offset correctly, return VDMA_OK
 * @note
 * @warning
 * @par       VDMA_UART1TX
 * @code      vdma_get_hw_read_point(VDMA_UART1TX, &write_offset);
 * @endcode
 */
vdma_status_t vdma_get_hw_write_offset(vdma_channel_t channel, uint32_t *write_offset);
