...
#define CVSD_DEC_PIC_EXPORT_API_NUM 3

/***********************************************************************/
static SemaphoreHandle_t cvsd_dec_load_lib_xSemaphore = NULL;
static preloader_pisplit_handle_t p_cvsd_dec_pisplit_library_handle = NULL;

/***********************************************************************/
extern void cvsd_dec_default_function_parameter(void);

void *cvsd_dec_decoder_export_parameters[CVSD_DEC_PIC_EXPORT_API_NUM] = {
...
void cvsd_dec_default_function_parameter(void)
{
...
uint32_t cvsd_dec_library_load(void *code_address, void *data_address, uint32_t *dram_pic_usage)
{
    ...
    load_info.library_callback = cvsd_dec_load_library_callback;
    load_info.library = &pisplit_cvsd_dec;
    load_info.dram_size = dram_pic_usage;
    preloader_pisplit_library_load_internal_port(&cvsd_dec_load_lib_xSemaphore, &p_cvsd_dec_pisplit_library_handle, &load_info);

    return 0;
}

uint32_t cvsd_dec_library_unload(void)
{
    preloader_pisplit_library_unload_internal_port(p_cvsd_dec_pisplit_library_handle, cvsd_dec_decoder_export_parameters, CVSD_DEC_PIC_EXPORT_API_NUM, cvsd_dec_default_function_parameter);
